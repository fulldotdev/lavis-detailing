---
title: Koplamp restauratie
description: Heldere koplampen voor beter zicht.
image:
  src: ../../../assets/exterieur1.webp
  alt: Koplamp restauratie

sections:
  - block: hero-7
    html: >
      <h1><strong>KOPLAMP RESTAURATIE:</strong> helder zicht en mooier uiterlijk</h1>
      <p>Vergeelde of doffe koplampen zijn niet alleen lelijk, maar ook gevaarlijk. Ze verminderen het licht en dus je zicht op de weg. Met onze professionele koplamp restauratie maken we je koplampen weer kristalhelder.</p>
    list:
      - Beter zicht en veiligheid
      - Helder en transparant
      - Verbetert het uiterlijk
      - Inclusief beschermlaag
    links:
      - text: Plan een gratis intake
        href: /intake/
    image:
      src: ../../../assets/exterieur1.webp
      alt: Koplamp restauratie

  - block: content-2
    html: >
      <h2>Waarom <strong>KOPLAMP RESTAURATIE?</strong></h2>
      <p>Door UV-straling en weersinvloeden worden koplampen na verloop van tijd mat en vergeeld. Dit vermindert de lichtopbrengst met wel 50%, wat gevaarlijk kan zijn bij slecht weer of 's nachts.</p>
      <p>Bij Lavis Detailing restaureren we je koplampen met professionele technieken. We schuren de beschadigde laag weg, polijsten de koplampen tot ze weer kristalhelder zijn, en brengen een beschermende coating aan die ze langer mooi houdt.</p>
    image:
      src: ../../../assets/exterieur2.webp
      alt: Koplamp voor en na

  - block: reviews-1
    html: <h2>Hier deden we het, nu <strong>JOUW AUTO.</strong></h2>
    refs:
      - /src/content/pages/reviews/reviews267.md
      - /src/content/pages/reviews/reviews220.md
      - /src/content/pages/reviews/reviews17.md
      - /src/content/pages/reviews/reviews106.md
      - /src/content/pages/reviews/reviews60.md
      - /src/content/pages/reviews/reviews99.md
      - /src/content/pages/reviews/reviews164.md
      - /src/content/pages/reviews/reviews5.md

  - block: tiles-6
    html: <h2>Perfect te combineren met</h2>
    refs:
      - /src/content/pages/diensten/exterieur-detailing.md
      - /src/content/pages/diensten/wassen.md
      - /src/content/pages/diensten/polijsten.md
seo:
  title: Koplamp restauratie | Lavis Detailing Sneek
  description: Herstel vergeelde en doffe koplampen voor beter zicht en uitstraling
---
