---
title: Bedankt
description: Bedankt voor je bericht
sections:
  - block: content-2
    html: >
      <h1>Bedankt voor je <strong>BERICHT!</strong></h1>
      <p>We hebben je bericht goed ontvangen en zullen zo snel mogelijk contact met je opnemen.</p>
      <p>Meestal reageren we binnen 24 uur. Check ook je spam folder voor het geval onze reactie daar terecht komt.</p>
      <p>Heb je een dringende vraag? Bel ons dan direct op <strong>+316 10158706</strong> of stuur een WhatsApp bericht.</p>
    image:
      src: ../../assets/exterieur1.webp
      alt: Lavis Detailing

  - block: tiles-6
    html: |
      <h2>Ontdek onze <strong>DIENSTEN</strong></h2>
      <p>Bekijk alvast wat we allemaal voor je kunnen betekenen.</p>
    refs:
      - /src/content/pages/diensten/exterieur-detailing.md
      - /src/content/pages/diensten/interieur-detailing.md
      - /src/content/pages/diensten/coating.md
      - /src/content/pages/diensten/polijsten.md

  - block: reviews-1
    html: <h2>Wat onze klanten <strong>ZEGGEN</strong></h2>
    refs:
      - /src/content/pages/reviews/reviews5.md
      - /src/content/pages/reviews/reviews17.md
      - /src/content/pages/reviews/reviews51.md
      - /src/content/pages/reviews/reviews60.md
      - /src/content/pages/reviews/reviews99.md
      - /src/content/pages/reviews/reviews106.md
      - /src/content/pages/reviews/reviews129.md
      - /src/content/pages/reviews/reviews170.md
seo:
  title: Bedankt voor je bericht | Lavis Detailing
  description: We hebben je bericht ontvangen en nemen zo snel mogelijk contact met je op.
---
